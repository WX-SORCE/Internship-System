 ...
 * @LastEditTime: 2025-04-02 15:53:27
 * @FilePath: \react-ant-admin\src\pages\list\aditfengkong.jsx
...
import { riskslist, risksave } from "@/api";
...
export default function SearchPage() {
    ...
    const getDataList = () => {
        const datalist = {
            page: 1,
            pageSize: 100,
            clientId: "100010000000",
            evaluatorId: userInfo.userId,
            resultLevel: "BALANCED",
            createdTimeStart: "2025-03-01 00:00:00",
            createdTimeEnd: "2025-12-31 00:00:00"
        }
        riskslist(datalist).then((res) => {
            console.log(res)
            if (res.code === 0) {
                let list = res.data;
                // if (res.data.tradeId) {
                ...
                let mapKey = [
                    // { title: "记录ID", dataIndex: "riskId", key: "riskId", ellipsis: true },
                    { title: "客户ID", dataIndex: "clientId", key: "clientId", ellipsis: true },
                    { title: "名下房产数量", dataIndex: "propertiesOwned", key: "propertiesOwned", ellipsis: true },
                    { title: "月收入", dataIndex: "monthlyIncome", key: "monthlyIncome", ellipsis: true },
                    { title: "贷款额", dataIndex: "loanAmount", key: "loanAmount", ellipsis: true },

                    // { title: "风险得分", dataIndex: "score", key: "score", ellipsis: true },
                    // { title: "附注", dataIndex: "remarks", key: "remarks", ellipsis: true },
                    // {
                    //     title: "审批意见",
                    //     key: "comment",
                    //     render: (text, record) => (
    ...
    const handleApprove = (record) => {
        // record.evaluatorId = userInfo.userId
        // record.remarks = "Client prefers low volatility",

        let yext = {
            evaluatorId: userInfo.userId,
            remarks: "Client prefers low volatility",
            state:1,
        }
        let ibj = { ...record, ...yext }
        risksave(ibj).then(res => {
        ...
        message.success(`已通过 ${record.clientId} 的审批`);
    };
